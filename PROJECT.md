...
```text
ai-settings/
├── skills/       # 309件のactive母集団をecc/codex/originalに分類
├── profiles/     # 採用セット
...
- `skills/original/`: このrepoで独自作成した6件。知識HTMLは構造skillとstyle skillを分離し、Harness固有処理はconformanceとreview customizationへ分ける。
- `profiles/harness-engineering/`: Harnessの設計、権限、評価、HTML reviewからのcustomizationを扱う専用profile。skillによってruntime authorityは拡張しない。
