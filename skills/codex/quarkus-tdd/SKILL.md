---
name: quarkus-tdd
description: "Test-driven development for Quarkus 3.x LTS using JUnit 5, Mockito, REST Assured, Camel testing, and JaCoCo. Use when adding features, fixing bugs, or refactoring event-driven services. Expected outcome: 該当技術の実装精度と検証効率を高める."
---

# Quarkus Tdd

Use when adding features, fixing bugs, or refactoring event-driven services. Aim to 該当技術の実装精度と検証効率を高める.
...
- push・deploy・送信・外部更新はread-only/dry-run既定、実行直前に明示承認 / Windows/PowerShell同等手順とLiteralPath・drive-letter・同一shell内の安全確認 / 813行の本文を入口、判断表、referencesへ分割し、最初に読む文脈量を抑える / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
