...
description: "Review prediction-market, basket, oracle, and trading-agent workflows for compliance, safety, data-quality, privacy, and execution risk. Use for Prediction Market Risk Review work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 欠陥を早期発見し、成果物の品質を高める."
---

# Prediction Market Risk Review

Use for Prediction Market Risk Review work that requires the source-specific procedures in the bundled domain guide. Aim to 欠陥を早期発見し、成果物の品質を高める.
...
- 開始・停止・handoff・完了条件を明文化し、会話要約だけで再開しない / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
