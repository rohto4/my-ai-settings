...
description: "Run team-based orchestration for agent squads using work items, ownership, agent Kanban, merge gates, and control pane handoffs. Use for Team Agent Orchestration work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 専門判断の精度を高め、調査の手戻りを減らす."
---

# Team Agent Orchestration

Use for Team Agent Orchestration work that requires the source-specific procedures in the bundled domain guide. Aim to 専門判断の精度を高め、調査の手戻りを減らす.
...
- Claude Codeのtool名・hook・.claude pathをCodexのtool/approval/commentary契約へ置換し、必要ならlegacy referenceへ分離する / subagentやparallel agentを既定起動せず、ユーザーまたはPJ方針が明示した場合だけ使うgateを追加する
