# Upstream domain guide: Delivery Gate
...
# Delivery Gate — Mechanical Quality Gate for Codex or the active coding agent
...
Codex or the active coding agent's built-in checks cover code quality (build → type → lint → test). But there's a different failure mode: the agent produces working code while the **session hygiene was neglected** — learning not captured, rationalized shortcuts, disk running out silently.
...
```bash
cp quality-gate.py <runtime-settings-dir>/scripts/
```

Add to `<runtime-settings-dir>/settings.json`:
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "python3 <runtime-settings-dir>/scripts/quality-gate.py",
        "timeout": 5000
      }]
    }]
  }
