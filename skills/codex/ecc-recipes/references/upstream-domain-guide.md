# Upstream domain guide: Ecc Recipes
...
```bash
for D in \
  "$HOME"/<project-agent-config>/plugins/marketplaces/ecc/commands \
  "$HOME"/<project-agent-config>/plugins/cache/ecc/ecc/*/commands \
  ./commands \
  ./<project-agent-config>/commands \
  "$HOME"/<project-agent-config>/commands; do
  [ -d "$D" ] && CMD_DIR="$D" && break
done
