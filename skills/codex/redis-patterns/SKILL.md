---
name: redis-patterns
description: "Redis data structure patterns, caching strategies, distributed locks, rate limiting, pub/sub, and connection management for production applications. Use for Redis Patterns work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 手順の抜け漏れを減らし、作業を再現しやすくする."
---

# Redis Patterns

Use for Redis Patterns work that requires the source-specific procedures in the bundled domain guide. Aim to 手順の抜け漏れを減らし、作業を再現しやすくする.
...
- 外部変更経路をfake/dry-run既定にし、実token・live API・deployを別gate化 / push・deploy・送信・外部更新はread-only/dry-run既定、実行直前に明示承認
