...
description: "Commercial-grade Python installer expert for Windows: Nuitka extreme compilation, dist slimming, DLL footprint analysis, and Inno Setup packaging to ship the smallest, fastest installers. Use for Generating Python Installer work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 設定漏れを減らし、導入を再現しやすくする."
---

# Generating Python Installer

Use for Generating Python Installer work that requires the source-specific procedures in the bundled domain guide. Aim to 設定漏れを減らし、導入を再現しやすくする.
...
- 外部変更経路をfake/dry-run既定にし、実token・live API・deployを別gate化 / 821行の本文を入口、判断表、referencesへ分割し、最初に読む文脈量を抑える
