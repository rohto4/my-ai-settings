# Upstream domain guide: Cost Tracking
...
Use this skill to analyze Codex or the active coding agent cost and usage history from the metrics log
that ECC's `stop:cost-tracker` hook writes.

## Where the data lives

The tracker appends one JSON object per session-stop to
`<runtime-settings-dir>/metrics/costs.jsonl`. Each row is a **cumulative snapshot for that
...
| `session_id` | Codex or the active coding agent session identifier |
