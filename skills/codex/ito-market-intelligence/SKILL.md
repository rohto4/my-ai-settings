...
description: "Research prediction-market events, venues, underliers, liquidity, and news context for Itô basket workflows. Use for Ito Market Intelligence work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 成果物の品質と一貫性を高める."
---

# Ito Market Intelligence

Use for Ito Market Intelligence work that requires the source-specific procedures in the bundled domain guide. Aim to 成果物の品質と一貫性を高める.
...
- token・Cookie・credentialは環境変数またはsecret storeから注入し、ログでmask / 完了条件・check/test・観測結果・未検証範囲を出力契約化 / 開始・停止・handoff・完了条件を明文化し、会話要約だけで再開しない / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
