...
description: "Discovers and invokes agent skills. Use when starting a session or when you need to discover which skill applies to the current task. Expected outcome: 専門判断の精度を高め、調査の手戻りを減らす."
---

# Using Agent Skills

Use when starting a session or when you need to discover which skill applies to the current task. Aim to 専門判断の精度を高め、調査の手戻りを減らす.
...
- 外部変更経路をfake/dry-run既定にし、実token・live API・deployを別gate化 / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
