# Upstream domain guide: Plan Orchestrate
...
Let `<claude-home>` denote the Codex or the active coding agent home directory: `<runtime-settings-dir>` on macOS/Linux, `%USERPROFILE%\.claude` on Windows. Resolve it the way the host platform resolves the user home directory (do not hardcode `~`).
