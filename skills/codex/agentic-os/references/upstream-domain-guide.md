# Upstream domain guide: Agentic Os
...
Treat Codex or the active coding agent as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside Codex or the active coding agent
- Setting up persistent Codex or the active coding agent automation that survives session restarts
...
├── <project-agent-config>/commands/  # Slash commands: user-facing CLI
...
| Commands (`<project-agent-config>/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
- Test runner access
- MCP servers as configured in `<project-agent-config>/mcp.json`
...
For parallel execution, use Codex or the active coding agent's background task capability or shell scripts that invoke Codex or the active coding agent with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `<project-agent-config>/commands/`. They define reusable workflows.
...
Place command files in `<project-agent-config>/commands/<command-name>.md`. Codex or the active coding agent auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not Codex or the active coding agent's built-in cron (which dies when the session ends).
...
Starting fresh every time Codex or the active coding agent opens.
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not Codex or the active coding agent's session cron
