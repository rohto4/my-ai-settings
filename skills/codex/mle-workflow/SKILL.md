---
name: mle-workflow
description: "Production machine-learning engineering workflow for data contracts, reproducible training, model evaluation, deployment, monitoring, and rollback. Use when building, reviewing, or hardening ML systems beyond one-off notebooks. Expected outcome: 定型作業と判断のばらつきを減らし、運用を安定させる."
---

# Mle Workflow

Use when building, reviewing, or hardening ML systems beyond one-off notebooks. Aim to 定型作業と判断のばらつきを減らし、運用を安定させる.
...
- 外部変更経路をfake/dry-run既定にし、実token・live API・deployを別gate化 / push・deploy・送信・外部更新はread-only/dry-run既定、実行直前に明示承認 / token・Cookie・credentialは環境変数またはsecret storeから注入し、ログでmask / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
