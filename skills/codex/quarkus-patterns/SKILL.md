...
description: "Quarkus 3.x LTS architecture patterns with Camel for messaging, RESTful API design, CDI services, data access with Panache, and async processing. Use for Quarkus Patterns work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 該当技術の実装精度と検証効率を高める."
---

# Quarkus Patterns

Use for Quarkus Patterns work that requires the source-specific procedures in the bundled domain guide. Aim to 該当技術の実装精度と検証効率を高める.
...
- push・deploy・送信・外部更新はread-only/dry-run既定、実行直前に明示承認 / 724行の本文を入口、判断表、referencesへ分割し、最初に読む文脈量を抑える
