...
description: "Local-evidence production readiness audit for shipped apps, pre-launch reviews, post-merge checks, and \"what breaks in prod?\" questions without sending repo data to an external audit service. Use for Production Audit work that requires the source-specific procedures in the bundled domain guide. Expected outcome: 判断材料を整理し、重要な見落としを減らす."
---

# Production Audit

Use for Production Audit work that requires the source-specific procedures in the bundled domain guide. Aim to 判断材料を整理し、重要な見落としを減らす.
...
- 外部変更経路をfake/dry-run既定にし、実token・live API・deployを別gate化 / 開始・停止・handoff・完了条件を明文化し、会話要約だけで再開しない / 長期作業はAGENTS.md/PROJECT.mdとdocs/impを実体再読し、進行中と完了証拠を分離
