...
Treat audit output as read-only evidence. Do not run `npm audit fix`, `npm update`, or another dependency-changing command during verification. Create a separate remediation change, inspect the proposed package and lockfile diff, and obtain any authorization required by the repository.
