# Legacy Hookify Format

This reference preserves the upstream Claude-specific Hookify format. Read it only when the user explicitly asks to maintain that legacy runtime; do not treat its paths or commands as Codex capabilities.
...
- Before delete, reset, or cleanup, resolve every absolute target path and obtain explicit approval. Keep diagnosis read-only and never pass targets enumerated in one shell to a destructive command in another shell.
