...
- 同期する場合は、対象profile検証、`Sync-AgentSkills.ps1 -WhatIf`、実同期、manifestとsource/destination hash一致の順で確認する。`.system`と母集団外skillは変更しない。
...
- `skills/` を変更したら対象profileを検証する。上記のruntime配布条件を満たす場合だけ同期し、manifestとsource/destination hash一致を確認する。groupをまたぐ同名skillは作らない。
